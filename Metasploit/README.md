...
Then we can select the `TRUN` module with the [`use`](https://www.offsec.com/metasploit-unleashed/msfconsole-commands/#use) command followed by the index of the module we want to use. In my case this was `8`.
...
3. Set the payload to be `windows/shell_reverse_tcp` if it has not already been done.
...
> [!IMPORTANT] 
> You will need to replace the ROP chain function in the Metasploit code. The rop_chains.txt generated by mona.py will contain the ruby function you can use! 