...
```
* `lpAddress`: This is the address of the starting page in the region who's address we would like to modify the protections of.
...
Based on the signature of `VirtualProtect()`, there are a few arguments we need to set. The first being the address of the page region we would like to modify, followed by the size of the region and then the permissions we would like to set. As we are not too concerned with the previous permissions set, we just need to set the `lpflOldProtect` value such that we do not overwrite the shellcode we have injected. 
...
4. Modify the exploit program to reflect [exploit0.py](./SourceCode/exploit0.py), this contains placeholders we will fill in at runtime, later these placeholders will be replaced and handled by the ROP chain.
   ...
   3. Observe the stack before we have stepped into the `RETN` instruction.
      ...
      * *Note*: Unless you see the TRUN instruction in the EAX register (Sometimes the function we select the `RETN` instrunction from is called in the base program).
