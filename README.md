...
The goal of our exploit will be to use a ROP chain to disable the NX-Bit and the associated [DEP](https://github.com/DaintyJet/VChat_DEP) protections on a memory page of the executing program by calling the [`VirtualProtect(...)`](https://learn.microsoft.com/en-us/windows/win32/api/memoryapi/nf-memoryapi-virtualprotect) function which changes the protections placed on a region of memory. In this case we will aim to call the `VirtualProtect(...)` function in the region our stack resides in. An alternative mentioned is using the `VirtualAlloc(...)` function to allocate space on our existing heap with the execution bit set allowing us to load shellcode to that location, then jump to and execute it.  
...
```
* `lpAddress`: This is the address of the starting page in the region whose address we would like to modify the protections of.
* `dwSize`: This is the size of the region we would like to modify the protections of. 
...
Based on the signature of `VirtualProtect()`, there are a few arguments we need to set. The first is the address of the page region we would like to modify, followed by the size of the region and then the permissions we would like to set. As we are not too concerned with the previous permissions set, we just need to set the `lpflOldProtect` value so that we do not overwrite the shellcode we have injected. 


We will need to setup the stack such that it has the following signature. *Note*: The stack is drawn such that the lower addresses are lower in the image, oftentimes this stack could be oriented in the opposite direction when drawn.
...
3. Locate a RETN instruction address, and pick one that does not have the (READONLY) flag set.
   ...
   <img src="Images/I22.png" width=800>
      ...
      * In this case I would choose `0x6250129D`.
...
4. Modify the exploit program to reflect [exploit0.py](./SourceCode/exploit0.py), this contains placeholders we will fill in at runtime, later these placeholders will be replaced and handled by the ROP chain.
   ...
   3. Observe the stack before we have stepped into the `RETN` instruction.
      ...
      * Notice that the address we jump to will be `VirtualProtect(...)`.
      * Notice that the return address for the function has also been written.

   4. Observe the stack after we have stepped into the `RETN` instruction.

		<img src="Images/I15.png" width=600>

      * Notice that we have a call stack in the `VirtualProtect(...)` function.
      * We will need to modify the values on the stack in order to test that we can call into Virtual Protect.

   6. Modify value of the `lpAddress` argument (Right click the argument in the stack view [Bottom Right Window] and select "modify"), we can give it the value currently held in the `ESP` register which is the stack address we had loaded the values in.

		<img src="Images/I16.png" width=600>

   7. Modify the value of the `dwSize` argument. This can be set to something reasonable like 512 (0x200).
  	
      <img src="Images/I17.png" width=600>

   8. Modify the value of the `flNewProtect` to be `0x00000040`.

      <img src="Images/I18.png" width=600>

   8. Modify the value of the `lpflOldProtect` to be a value above us on the stack in this case I chose `0x0131F9D0`.

      <img src="Images/I19.png" width=600>
   
      * Notice the address we chose was on the stack, this is because the stack has READ and WRITE permissions, so if we were to leave the address we had there before which is located in the TEXT segment, if the code were to execute as is VChat would crash after raising an exception; if it does recover it would later crash when it jumps back to the stack and attempts to execute instructions while DEP is still enabled. 

   9. Step through the function for a bit to ensure that it works, once we are sure the function call has occurred correctly we can move onto the next step. 
...
6. Now, we can generate a ROP chain that will fill the stack with the appropriate values *Dynamically* such that the manual modification of stack values with a debugger is unneeded. This will leverage the `PUSHAD` x86 assembly command, this is equivalent to the following assembly instructions and shows that it will push all of the General Purpose registers onto the stack. 
   ...
   2. We will want to load the registers in such a way that the stack has the following signature. 
      ...
      * The `ECX` register will contain the value of the `lpflOldProtect` argument, which is the address of the old status and permissions of the memory region we have specified will be written.
...
1. Using the [mona.py](https://github.com/corelan/mona) command we generate a new ROP chain, this is necessary due to the use of gadgets that are located in modules with ASLR is enabled. 
   ```
   !mona rop -m *.dll -n
   ```
   Generated ROP chain:
